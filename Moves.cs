...
namespace ForcedChess
{
    class Moves
    {
		internal static long FILE_A = 72340172838076673L;
		internal static long FILE_H = -9187201950435737472L;
		internal static long FILE_AB = 217020518514230019L;
		internal static long FILE_GH = -4557430888798830400L;
		internal static long RANK_1 = -72057594037927936L;
		internal static long RANK_4 = 1095216660480L;
		internal static long RANK_5 = 4278190080L;
		internal static long RANK_8 = 255L;
		internal static long CENTRE = 103481868288L;
		internal static long EXTENDED_CENTRE = 66229406269440L;
		internal static long KING_SIDE = -1085102592571150096L;
		internal static long QUEEN_SIDE = 1085102592571150095L;
		internal static long KING_SPAN = 460039L;
		internal static long KNIGHT_SPAN = 43234889994L;
		internal static long NOT_MY_PIECES;
		internal static long MY_PIECES;
		internal static long OCCUPIED;
		internal static long EMPTY;
		internal static int[] CASTLE_ROOKS = new int[] {63, 56, 7, 0};

        internal static long[] RankMasks8 = new long[] { 0xFFL, 0xFF00L, 0xFF0000L, 0xFF000000L, 0xFF00000000L, 0xFF0000000000L, 0xFF000000000000L, unchecked((long)0xFF00000000000000L) }; //from rank1 to rank8
        internal static long[] FileMasks8 = new long[] { 0x101010101010101L, 0x202020202020202L, 0x404040404040404L, 0x808080808080808L, 0x1010101010101010L, 0x2020202020202020L, 0x4040404040404040L, unchecked((long)0x8080808080808080L) }; //from fileA to FileH
        internal static long[] DiagonalMasks8 = new long[] { 0x1L, 0x102L, 0x10204L, 0x1020408L, 0x102040810L, 0x10204081020L, 0x1020408102040L, 0x102040810204080L, 0x204081020408000L, 0x408102040800000L, 0x810204080000000L, 0x1020408000000000L, 0x2040800000000000L, 0x4080000000000000L, unchecked((long)0x8000000000000000L) }; //from top left to bottom right
        internal static long[] AntiDiagonalMasks8 = new long[] { 0x80L, 0x8040L, 0x804020L, 0x80402010L, 0x8040201008L, 0x804020100804L, 0x80402010080402L, unchecked((long)0x8040201008040201L), 0x4020100804020100L, 0x2010080402010000L, 0x1008040201000000L, 0x804020100000000L, 0x402010000000000L, 0x201000000000000L, 0x100000000000000L }; //from top right to bottom left

        internal static long HAndVMoves(int s)
        {
        ...
        internal static long DAndAntiDMoves(int s)
        {
            //REMINDER: requires OCCUPIED to be up to date
            long binaryS = 1L << s;
            long possibilitiesDiagonal = ((OCCUPIED & DiagonalMasks8[(s / 8) + (s % 8)]) - (2 * binaryS)) ^ BitOps.Reverse(BitOps.Reverse(OCCUPIED & DiagonalMasks8[(s / 8) + (s % 8)]) - (2 * BitOps.Reverse(binaryS)));
            long possibilitiesAntiDiagonal = ((OCCUPIED & AntiDiagonalMasks8[(s / 8) + 7 - (s % 8)]) - (2 * binaryS)) ^ BitOps.Reverse(BitOps.Reverse(OCCUPIED & AntiDiagonalMasks8[(s / 8) + 7 - (s % 8)]) - (2 * BitOps.Reverse(binaryS)));
        ...
        public static long makeMove(long board, String move, char type) {
            ...
            {//'regular' move
                ...
                if ((BitOps.getUnsignedRightShift(board, start)&1)==1) 
                {
                    board &= ~(1L<<start); 
                    board |= (1L<<end);
                } 
                ...
                {
                    start=BitOps.numberOfTrailingZeros(FileMasks8[move[0]-'0']&RankMasks8[1]);
                    end=BitOps.numberOfTrailingZeros(FileMasks8[move[1]-'0']&RankMasks8[0]);
                } 
                else 
                {
                    start=BitOps.numberOfTrailingZeros(FileMasks8[move[0]-'0']&RankMasks8[6]);
                    end=BitOps.numberOfTrailingZeros(FileMasks8[move[1]-'0']&RankMasks8[7]);
                }
                if (type==move[2]) 
                {
                    board |= (1L<<end);
            ...
            {//en passant
                ...
                if (move[2]=='W') 
                {
                    start=BitOps.numberOfTrailingZeros(FileMasks8[move[0]-'0']&RankMasks8[3]);
                    end=BitOps.numberOfTrailingZeros(FileMasks8[move[1]-'0']&RankMasks8[2]);
                    board &= ~(FileMasks8[move[1]-'0']&RankMasks8[3]);
                } else {
                    start=BitOps.numberOfTrailingZeros(FileMasks8[move[0]-'0']&RankMasks8[4]);
                    end=BitOps.numberOfTrailingZeros(FileMasks8[move[1]-'0']&RankMasks8[5]);
        ...
        public static long makeMoveCastle(long rookBoard, long kingBoard, String move, char type) {
            int start=(int)(Char.GetNumericValue(move[0])*8)+(int)(Char.GetNumericValue(move[1]));
            if (((BitOps.getUnsignedRightShift(kingBoard,start)&1)==1)&&
        ...
        public static long makeMoveEP(long board,String move) {
            if (Char.IsDigit(move[3])) {
                int start=(int)(Char.GetNumericValue(move[0])*8)+(int)(Char.GetNumericValue(move[1]));
                if ((Math.Abs(move[0]-move[2])==2)&&((BitOps.getUnsignedRightShift(board,start)&1)==1)) 
                {//pawn double push
        ...
        public static String possibleWP(long WP,long BP,long EP) {
            ...
            while (possibility != 0)
            {
                int index = BitOps.numberOfTrailingZeros(possibility);
        ...
        public static String possibleN(long OCCUPIED,long N) {
            ...
            while(i != 0)
            {
                int iLocation=BitOps.numberOfTrailingZeros(i);
                if (iLocation>18)
                {
                ...
                while (j != 0)
                {
                    int index=BitOps.numberOfTrailingZeros(j);
        ...
        public static String possibleK(long OCCUPIED,long K) {
            String list="";
            long possibility;
            int iLocation=BitOps.numberOfTrailingZeros(K);
            if (iLocation>9)
            {
        ...
        public static String possibleCW(long WP,long WN,long WB,long WR,long WQ,long WK,
            long BP,long BN,long BB,long BR,long BQ,long BK,bool CWK,bool CWQ) {
            String list="";
        ...
        public static String possibleCB(long WP,long WN,long WB,long WR,long WQ,long WK,
            long BP,long BN,long BB,long BR,long BQ,long BK,bool CBK,bool CBQ) {
            String list="";
        ...
        public static long unSafeForBlack(long WP,long WN,long WB,long WR,long WQ,long WK,
            ...
            OCCUPIED=WP|WN|WB|WR|WQ|WK|BP|BN|BB|BR|BQ|BK;
            //pawn
            unSafe=(BitOps.getUnsignedRightShift(WP, 7)&~FILE_A);//pawn capture right
            unSafe|=(BitOps.getUnsignedRightShift(WP, 9)&~FILE_H);//pawn capture left
            long possibility;
            //knight
            long i=WN&~(WN-1);
            int iLocation;
            while(i != 0)
            {
                iLocation=BitOps.numberOfTrailingZeros(i);
                if (iLocation>18)
                {
                    possibility=KNIGHT_SPAN<<(iLocation-18);
                }
            ...
            //king
            iLocation=BitOps.numberOfTrailingZeros(WK);
            if (iLocation>9)
            {
                possibility=KING_SPAN<<(iLocation-9);
            }
        ...
        public static long unSafeForWhite(long WP,long WN,long WB,long WR,long WQ,long WK,
            ...
            //king
            iLocation=BitOps.numberOfTrailingZeros(BK);
            if (iLocation>9)
            {
                possibility=KING_SPAN<<(iLocation-9);
            }
        ...
        //    }
        //    for (int i=0;i<8;i++) {
        //        Console.WriteLine(Arrays.toString(chessBoard[i]));
        //    }
        //}
    }
}
